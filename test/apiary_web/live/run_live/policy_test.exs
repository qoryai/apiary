defmodule ApiaryWeb.RunLive.PolicyTest do
  ...
  defp connections(conn, run) do
    {:ok, view, _html} = live(conn, ~p"/workspace/runs/#{run.run_id}/connections")
    view
  end
  ...
  describe "the header's policy cell (pd9)" do
    test "the version the run reported links to that exact version", %{conn: conn, scope: scope} do
      ...
      {:ok, view, _html} = live(conn, ~p"/workspace/runs/#{run.run_id}")

      path = "/workspace/policy/targets/#{target.id}/versions/#{configuration.version}"
    ...
    test "a digest of the workspace's baseline says so", %{conn: conn, scope: scope} do
      run = policy_run(scope, applied: @other)
      enforce(scope)
      ...
      assert has_element?(
               view,
               ~s(#run-facts a.q-ver[href="/workspace/policy/versions/#{configuration.version}"])
             )
    ...
    test "behind a target's version while on the baseline's: both numberings are named", %{
      ...
      path = "/workspace/policy/targets/#{target.id}/versions/1"
      ...
      {:ok, view, _html} = live(conn, ~p"/workspace/runs/#{run.run_id}/details")
    ...
    test "an alive run that reported another digest is behind, until it reports the one in force",
      ...
      compare =
        "/workspace/policy/targets/#{target.id}/versions/#{new.version}?compare=#{old.version}"
    ...
    test "a workspace whose policy nobody has made shows no version and no drift", %{
      conn: conn,
      scope: scope
    } do
  ...
  describe "the timeline (pe6)" do
    test "a second policy applied is a reload, with the delta of the two events", %{
      ...
      assert has_element?(view, ~s(#e-3 a.q-ver[href="/workspace/policy/versions/#{v1.version}"]))
      ...
      assert has_element?(
               view,
               ~s(#e-30 a.q-ver[href="/workspace/policy/versions/#{v2.version}"])
             )
    ...
    test "a reload's deny list: the count on the item, chips with the deny mark, the Details",
      ...
      {:ok, _view, html} = live(conn, ~p"/workspace/runs/#{run.run_id}/details")
  ...
  describe "the slot of a row (pd8)" do
    ...
    test "allowing keeps the record as it was and adds the line after", %{
      ...
      rule = "/workspace/policy/targets/#{target.id}?rule=files.cdn.example"
    ...
    test "denying for the workspace replaces the workspace's rule, and the row stays allowed", %{
      conn: conn,
      scope: scope,
      run: run
    } do
      ...
      assert has_element?(view, ~s(tr#cx-#{id}[data-decision=allowed]))
      assert text(view, "#cx-#{id}-after") =~ "Denied for the workspace"

      assert has_element?(
               view,
               ~s(a#cx-#{id}-act[href="/workspace/policy?rule=registry.example"])
             )
    end
    ...
    test "a locked rule: the owner is told where to change it, and there is no form", %{
      ...
      assert refusal =~ "You can change or unlock it on the workspace's policy page."

      assert has_element?(
               view,
               ~s(#rule-popover-locked-rule[href="/workspace/policy?rule=%2A.paste.example"])
             )
  ...
  describe "the popover says what the chosen scope will get" do
    ...
    test "a path for the target that holds the host to paths, the whole host for the workspace that does not",
         %{conn: conn, scope: scope} do
      enforce(scope)
      ...
      html = render(view)
      assert html =~ "api.pathed.example is allowed for the workspace."
  ...
  describe "tenancy of the row's events" do
    test "a connection of another workspace is not found, whatever the event names", %{
      conn: conn,
      scope: scope
    } do
      enforce(scope)
