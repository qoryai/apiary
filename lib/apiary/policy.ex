defmodule Apiary.Policy do
  @moduledoc """
  The security policy of a workspace and of its targets, the run configurations rendered
  ...
  A holder is whose rules, mode and run configurations a call is about: `nil` or
  `:workspace` for the workspace's baseline, or an `Apiary.Runs.Target` of the scope's
  workspace. Another workspace's target, rule or configuration is not found.
  """
  ...
  @doc "The topic of a workspace's policy: `\"policy:<workspace_id>\"`."
  def topic(workspace_id), do: "policy:#{workspace_id}"

  @doc "Subscribes the caller to the policy of the scope's workspace."
  ...
  the pages: "machines use their own policy until the first change here".
  """
  ...
  @doc """
  The workspace's targets, by system and path, each with `rule_count`, how many rules of
  its own it has (a virtual count on the map, not the struct), `own_mode`, the mode it
  set for itself or nil when it follows the workspace, and `mode`, the one in force for
  it: `[%{target: …, rule_count: n, own_mode: … | nil, mode: …}]`. At most 500.
  """
  ...
  def list_targets(
        %Scope{workspace: %Workspace{id: workspace_id}, organisation: %Organisation{id: org_id}} =
          scope
      ) do
    ...
    Repo.all(
      from p in Target,
        where: p.workspace_id == ^workspace_id and p.organisation_id == ^org_id,
        left_join: r in Rule,
        on: r.target_id == p.id,
        group_by: p.id,
    ...
    |> Enum.map(fn %{target: target} = row ->
      Map.merge(row, %{
        ...
        mode: target.egress_mode || workspace_mode
      })
    end)
  end

  @doc "One target of the scope's workspace by id."
  ...
  def get_target(%Scope{} = scope, id) do
    with {:ok, id} <- Ecto.UUID.cast(id),
      ...
      _ -> {:error, not_found(gettext("This workspace has no such target."))}
    end
  end
  ...
  @doc "The mode of the workspace's policy, `\"observe\"` or `\"enforce\"`, as it is now."
  ...
  @doc """
  Sets the mode of the workspace: `set_mode(scope, nil, mode)`. An owner's act in either
  direction; a member is refused with a sentence. The workspace's mode is the default of
  its targets: a change renders the baseline and every target that follows the workspace
  ...
  @doc """
  Sets the mode of the workspace (`nil` or `:workspace`: `{:ok, mode}`) or of a target
  (`{:ok, %{mode:, own:, workspace:}}`). A target takes `"observe"`, `"enforce"`, or
  `:inherit` (also `"inherit"`) to follow the workspace again, which is what every target
  does until somebody says otherwise. An owner's act, like the workspace's. It is a change
  of the target's policy (`mode_changed`, with the target's own mode, or
  ...
  def set_mode(%Scope{}, holder, _mode) when holder in [nil, :workspace] do
  ...
  def set_mode(%Scope{}, _holder, _mode) do
    {:error,
     Error.new(
       :invalid,
       gettext("A target's mode is observe, enforce, or the workspace's."),
       :mode
     )}
  end
  ...
  def list_rules(%Scope{} = scope, holder) do
    case holder_id(scope, holder) do
      {:ok, target_id} -> rules(scope.workspace.id, target_id)
      {:error, _not_found} -> []
    end
  end

  @doc "One rule of the scope's workspace by id."
    ...
    with {:ok, id} <- Ecto.UUID.cast(id),
         %Rule{} = rule <-
           Repo.one(
             from r in Rule,
               where:
                 r.id == ^id and r.workspace_id == ^workspace_id and r.organisation_id == ^org_id
           ) do
      {:ok, rule}
    else
      _ -> {:error, not_found(gettext("This workspace has no such rule."))}
    end
  end
  ...
  holder are taken as the start, so a target that adds a path keeps the workspace's and
  from then on has a list of its own. Refused when the host is already reached on every
  path, and in a target when the workspace's rule for the host is locked.
  """
  ...
  target (`:target`) or in the workspace (`:workspace`). When the host is held to paths in
  the holder and the connection names a path, the path is added to or taken out of those
  paths instead. The connection is one of the scope's workspace.
  """
  @spec rule_from_connection(Scope.t(), Connection.t(), :allow | :deny, :target | :workspace) ::
          {:ok, Rule.t()} | refusal
  ...
  Read from `connections` by the workspace and when they were last seen, at most
  ...
  def uncovered(%Scope{workspace: %Workspace{}} = scope, %DateTime{} = since),
    do: Activity.uncovered(scope, nil, since)

  @doc """
  `uncovered/2` for a holder. For the workspace (`nil`, `:workspace`) it is `uncovered/2`:
  what enforcing the workspace would start denying, so only the runs of targets that
  follow the workspace's mode count, and the runs that name no target; a target with a
  mode of its own would not change. For a target: what enforcing that target would start
  denying, from its own runs under its own effective rules, whatever its mode is now.
  A target that is not the workspace's has nothing.
  """
  @spec uncovered(Scope.t(), holder, DateTime.t()) :: {:ok, [uncovered]} | :unavailable
  def uncovered(%Scope{workspace: %Workspace{}} = scope, holder, %DateTime{} = since) do
    case holder_id(scope, holder) do
  ...
  def denied_summary(%Scope{workspace: %Workspace{}} = scope, %DateTime{} = since),
  ...
  def denied_destinations(%Scope{workspace: %Workspace{}} = scope, %DateTime{} = since),
    do: Activity.denied_destinations(scope, since)

  @doc """
  What the workspace overview reads of the connections in one bounded read: `denied`, the
  ...
  @spec rule_activity(Scope.t(), holder, DateTime.t()) ::
          ...
          | :unavailable
  def rule_activity(%Scope{workspace: %Workspace{}} = scope, holder, %DateTime{} = since) do
    case holder_id(scope, holder) do
  ...
  @doc """
  How many declared hosts across the workspace's targets no rule covers or denies, counted
  ...
  workspace's rules, read once. Two bounded reads and the workspace's mode, however many
  targets the workspace has; no attempts are counted, so nothing here can be unavailable.
  """
  ...
  def declared_hosts(%Scope{} = scope, %Target{} = target, since \\ nil) do
    ...
    case holder_id(scope, target) do
      {:ok, target_id} ->
        Suggestions.report(scope.workspace.id, target_id, effective(scope, target), since)
  ...
  row says so by its `target_id`. A workspace nobody has changed yet (`managed?/1` is
  ...
  def current_configuration(%Scope{workspace: %Workspace{} = workspace} = scope, holder) do
    with {:ok, target_id} <- holder_id(scope, holder) do
      in_force(workspace.organisation_id, workspace.id, target_id)
    end
  end
  ...
  def get_configuration(%Scope{} = scope, holder, version) do
    with {:ok, target_id} <- holder_id(scope, holder),
         ...
         %RunConfiguration{} = configuration <-
           Repo.one(
             from c in configurations(scope.workspace.id, target_id), where: c.version == ^version
           ) do
      {:ok, configuration}
    else
  ...
  def configuration_for_digest(%Scope{} = scope, holder, digest) when is_binary(digest) do
    with {:ok, target_id} <- holder_id(scope, holder),
         %RunConfiguration{} = configuration <-
           by_digest(scope.workspace.id, target_id, digest) ||
             (target_id && by_digest(scope.workspace.id, nil, digest)) do
      {:ok, configuration}
    else
      _ ->
        {:error,
         not_found(gettext("This workspace served no run configuration with that digest."))}
    end
  end

  def configuration_for_digest(%Scope{}, _holder, _digest),
    do:
      {:error, not_found(gettext("This workspace served no run configuration with that digest."))}
  ...
  def list_configurations(%Scope{} = scope, holder, page \\ 1) do
    case holder_id(scope, holder) do
      {:ok, target_id} ->
        configurations(scope.workspace.id, target_id)
  ...
  `applied` is what its last policy applied event named; `in_force` is nil for a workspace
  ...
  yet, and false when it reported none: such a run holds no fetched configuration. A run
  of another workspace is a refusal, `{:error, %Apiary.Policy.Error{reason: :not_found}}`,
  not a map.
  """
  @spec digests(Scope.t(), Run.t()) ::
          ...
          | refusal
  def digests(
        %Scope{workspace: %Workspace{id: workspace_id} = workspace},
        %Run{workspace_id: workspace_id} = run
      ) do
    in_force =
      case in_force(workspace.organisation_id, workspace_id, run.target_id) do
  ...
  def digests(%Scope{}, %Run{}),
    do: {:error, not_found(gettext("This workspace has no such run."))}
  ...
  target's id, or nil for the baseline. `holders` holds `nil` or `:workspace` for the
  baseline, targets, or target ids; at most #{@bulk_max} are read. A holder with no
  configuration of its own (a target served the baseline's, another workspace's, an id
  that is none) has no key.
  """
  @spec newest_versions(Scope.t(), [holder | Ecto.UUID.t()]) :: %{
          optional(Ecto.UUID.t() | nil) => RunConfiguration.t()
        }
  def newest_versions(%Scope{workspace: %Workspace{id: workspace_id}}, holders)
  ...
  target. A change of the workspace may have rendered the baseline and several
  targets; one that rendered the same bytes made none and has no key, and neither
  has another workspace's change. At most #{@bulk_max} ids are read.
  """
  @spec configurations_for_changes(Scope.t(), [Ecto.UUID.t()]) :: %{
          optional(Ecto.UUID.t()) => [RunConfiguration.t()]
        }
  def configurations_for_changes(%Scope{workspace: %Workspace{id: workspace_id}}, change_ids)
  ...
  A holder nobody has changed, or another workspace's, has no key.
  """
  @spec last_changes(Scope.t(), [holder | Ecto.UUID.t()]) :: %{
          optional(Ecto.UUID.t() | nil) => Change.t()
        }
  def last_changes(%Scope{workspace: %Workspace{id: workspace_id}}, holders)
      when is_list(holders) do
    {ids, baseline?} = holder_keys(holders)
  ...
  defp holder_keys(holders) do
    ...
    baseline? = Enum.any?(holders, &(&1 in [nil, :workspace]))

    ids =
      uuids(
        for holder <- holders, holder not in [nil, :workspace] do
          case holder do
            %Target{id: id} -> id
            id -> id
          end
  ...
  #{@page_size}, `changed_by` preloaded. `:all` as the holder lists every change of the
  workspace, `target` preloaded.
  """
  ...
  @doc "One change of the scope's workspace by id, `changed_by` and `target` preloaded."
  ...
  defp room(_workspace_id, _target_id, %Rule{}), do: :ok

  defp room(workspace_id, target_id, nil) do
    if length(rules(workspace_id, target_id)) < @rules_max do
      :ok
    else
      {:error,
       Error.new(
  ...
  defp store(workspace, user, target_id, nil, candidate, _attrs) do
    rule =
      Repo.insert!(%{
        candidate
  ...
  # The paths in force are read under the workspace's lock, so a path added here is added
  # to what is there now and not to what was there when the page was drawn.
  ...
  defp not_locked_above(%Effective{entries: entries}, _target_id, host) do
    if Enum.any?(
         entries,
         &(&1.kind == :host and &1.host == host and &1.source == :workspace and &1.locked)
       ) do
      {:error,
       Error.new(
         :locked,
         gettext(
           "The workspace's rule for %{host} is locked, so a target cannot change its paths. An owner changes it in the workspace.",
           host: host
         ),
         :host
       )}
  ...
  # The rule as it is under the workspace's lock:
  # one removed in the meantime is not found.
  defp reread(%Workspace{id: workspace_id}, %Rule{id: id}) do
    case Repo.one(from r in Rule, where: r.id == ^id and r.workspace_id == ^workspace_id) do
      %Rule{} = rule -> {:ok, rule}
      nil -> {:error, not_found(gettext("This rule is gone: somebody removed it a moment ago."))}
    end
  end

  # One write: the workspace's row is locked first, so writes of one workspace happen one
    ...
    case result do
      ...
      {:ok, {value, %Change{} = change}} ->
        Phoenix.PubSub.broadcast(
          ...
          topic(workspace.id),
          {:policy_changed,
           %{workspace_id: workspace.id, target_id: change.target_id, action: change.action}}
        )
  ...
  # The mode is the holder's own: the workspace's for the baseline, and for a target the
  # one it set or "inherit", so its history shows its own changes and not the workspace's.
  defp snapshot_mode(%Workspace{} = workspace, nil), do: workspace.egress_mode

  defp snapshot_mode(%Workspace{}, target_id) do
    Repo.one(from p in Target, where: p.id == ^target_id, select: p.egress_mode) ||
      "inherit"
  end

  defp snapshot(%Workspace{} = workspace, target_id) do
    %{
      "mode" => snapshot_mode(workspace, target_id),
      "rules" =>
        for rule <- rules(workspace.id, target_id) do
          %{
            "kind" => rule.kind,
            "action" => rule.action,
            "host" => rule.host,
  ...
  @doc """
  Renders the documents of every managed workspace again through today's resolution, in
  the workspace's lock, for `mix apiary.policy.rerender`: a holder whose bytes change gets
  a new version and a `rerendered` change (no change of the rules: `before` equals
  `after`), and unchanged bytes write nothing. What an upgrade that changed what a render
  says (a release that put `deny` in the document) needs once.
  `%{workspaces: n, versions: m}`, the workspaces visited and the versions written.
  """
  ...
  def rerender_all do
    ...
    |> Enum.reduce(%{workspaces: 0, versions: 0}, fn workspace_id, totals ->
      case rerender(workspace_id) do
        {:ok, versions} ->
          %{workspaces: totals.workspaces + 1, versions: totals.versions + versions}
  ...
  @doc false
  # One workspace: in its lock, as any write.
  # Nothing is announced when nothing was written.
  def rerender(workspace_id) do
    result =
      Repo.transact(fn ->
        workspace =
          Repo.one!(from h in Workspace, where: h.id == ^workspace_id, lock: "FOR NO KEY UPDATE")

        if managed_workspace?(workspace.id) do
    ...
    case result do
      ...
      {:ok, versions} ->
        Phoenix.PubSub.broadcast(
          Apiary.PubSub,
          topic(workspace_id),
          {:policy_changed, %{workspace_id: workspace_id, target_id: nil, action: "rerendered"}}
        )

        {:ok, versions}

      {:error, %Error{} = error} ->
        Logger.warning("policy rerender refused workspace=#{workspace_id}: #{error.message}")
        {:error, error}
    end
  end
  ...
  defp render_holders(%Workspace{} = workspace, fun) do
    ...
    own =
      Repo.all(from r in Rule, where: r.workspace_id == ^workspace.id and not is_nil(r.target_id))
      |> Enum.group_by(& &1.target_id)

    rendered =
      Repo.all(
        from c in RunConfiguration,
          where: c.workspace_id == ^workspace.id and not is_nil(c.target_id),
          distinct: true,
          select: c.target_id
      )
    ...
    modes =
      Repo.all(
        from p in Target,
          where: p.workspace_id == ^workspace.id and not is_nil(p.egress_mode),
  ...
  defp store(workspace, change, target_id, document) do
    digest = Render.digest(document)
  ...
  defp elsewhere(%Error{} = error, _workspace, %Change{}, nil),
    do: %{
      error
      | message: gettext("In the workspace's baseline: %{refusal}", refusal: error.message)
    }

  defp elsewhere(%Error{} = error, workspace, %Change{}, target_id) do
    case Repo.one(from p in Target, where: p.id == ^target_id and p.workspace_id == ^workspace.id) do
  ...
  @doc false
  # The configuration in force for a target of the workspace, or the baseline's: a read,
  # and nothing but a read. A workspace nobody has changed has no baseline row, and none
  # is made here: the first version is written by the first change and by nothing else
  # (`render_all/3`), so what a page shows as version 1 is what the first change made.
  def in_force(_organisation_id, workspace_id, target_id) do
    own = target_id && Repo.one(newest(workspace_id, target_id))

    case own || Repo.one(newest(workspace_id, nil)) do
  ...
  defp unmanaged do
    Error.new(
      :unmanaged,
      gettext(
        "Nobody has made this workspace's policy yet: its machines run under their own, and there is no version until the first change here."
      )
    )
  end
  ...
  def newest(workspace_id, target_id) do
    from c in configurations(workspace_id, target_id), order_by: [desc: c.version], limit: 1
  end
  ...
  defp configurations(workspace_id, target_id) do
    from c in RunConfiguration,
      where:
  ...
  defp by_digest(workspace_id, target_id, digest) do
    Repo.one(
      from c in configurations(workspace_id, target_id),
  ...
  defp rules(workspace_id, target_id) do
    Repo.all(
      from r in Rule,
  ...
  defp targets(%Scope{
    ...
    from p in Target, where: p.workspace_id == ^workspace_id and p.organisation_id == ^org_id
  end

  # The target's id when it is one of the scope's workspace, nil for the baseline.
  defp holder_id(%Scope{}, holder) when holder in [nil, :workspace], do: {:ok, nil}
  ...
  defp holder_id(%Scope{} = scope, %Target{id: id}) when is_binary(id) do
    case Repo.one(from p in targets(scope), where: p.id == ^id, select: p.id) do
      nil -> {:error, not_found(gettext("This workspace has no such target."))}
      id -> {:ok, id}
    end
  end
  ...
  defp lock_is_the_workspaces(_target_id, true) do
    {:error,
     Error.new(
       :invalid,
       gettext(
         "Only a rule of the workspace can be locked: a lock is what holds it against the targets."
       )
     )}
  end

  defp lock_is_the_workspaces(_target_id, _locked), do: :ok
  ...
  defp connection_host(%Scope{workspace: %Workspace{id: workspace_id}}, %Connection{
  ...
  defp connection_host(%Scope{}, _connection),
    do: {:error, not_found(gettext("This workspace has no such connection."))}

  defp connection_holder(_scope, _connection, :workspace), do: {:ok, nil}
    ...
    target =
      Repo.one(
        from p in targets(scope),
          join: r in Run,
          on: r.target_id == p.id,
          where: r.id == ^run_id and r.workspace_id == ^scope.workspace.id
      )
    ...
    case target do
      ...
      nil ->
        {:error,
         Error.new(
           :not_found,
           gettext("This run names no target, so the rule has nowhere to go but the workspace.")
         )}
    end
  end
